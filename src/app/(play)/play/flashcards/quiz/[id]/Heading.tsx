...
type HeadingProps = {
  gameData: GameData;
  stopTimer: boolean;
  answered: { questionsLeft: number, correctAnswers?: number } | null;
...
const Heading = ({ gameData, stopTimer, answered, setAnswered, getQuestion } : HeadingProps) => {
  ...
  if (time === 0) {
    if (!answered) {
      setAnswered({questionsLeft: gameData.numberOfQuestions - gameData.actualQuestion - 1 });
    }
  }
  ...
  const handleEndGame = async () => {
    toast.promise(
      async () => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API}/play/flashcards/${gameData.id}/user/quiz/end`, {
          method: 'POST',
          cache: 'no-cache',
        });

...
const Playground = () => {
  ...
  const calculateHeight = () => {
    ...
    cardRef.current.style.height = 
      ...
      ? Number(conceptRef.current?.offsetHeight) > Number(cardRef.current?.offsetWidth) * 0.5625
        ? conceptRef.current?.offsetHeight + 'px'
        : window.innerWidth < 640
          ? Number(cardRef.current?.offsetWidth) + "px"
          : Number(cardRef.current?.offsetWidth) * 0.5625 + "px"
      : Number(definitionRef.current?.offsetHeight) > Number(cardRef.current?.offsetWidth) * 0.5625
